...
```{r}
NOMBRE_COMPLETO = "Artemio Baños Valdez"
DIGITOS_MATRICULA = 01022631
```
...
- Frecuencia absoluta del item {beer} (**NOTE-03**): fi(beer) = 3 
- Frecuencia absoluta del item {nuts} (**NOTE-04**): fi(nuts) = 3
- Frecuencia absoluta del itemset {beer, nuts} (**NOTE-05**): fi(nuts,beer) = 1
- Support del item {beer} (**NOTE-06**): sup(beer) = 3/5 = 0.6
- Support del itemset {beer, nuts} (**NOTE-07**): sup(beer,nuts) = 1/5 = 0.2 
...
- support (mínimo) (**NOTE-12**): support minimo = 0.1
- confidence (mínimo) (**NOTE-13**):confidence minimo = 0.8
- items (máximo) (**NOTE-14**): itesm maximo = 10
- tiempo de verificación de subsets (máximo) (**NOTE-15**):tiempo maximo = 5
...
- Media aritmética de support (**NOTE-17**): mean (support) = 0.2478
- Media aritmética de confidence (**NOTE-18**): mean (confidence) = 0.9957 
...
- ¿Cuál es la interpretación de la regla `{} => {diapers}` en la que se aprecia la ausencia del LHS? (**NOTE-19**): Significa que solamente compran pañales (diapers), es decir, que cuando no llevan nada màs o cuando solo llevan un artìculo, estan comprando unicamente pañales (diapers)."Cliente que no compra nada màs, compra pañales". 
- Calcula el valor de la métrica support para la regla `{beer} => {nuts}` (**NOTE-20**): support{beer} => {nuts} = 1/5 = 0.2
- Calcula el valor de la métrica confidence para la regla `{beer} => {nuts}` (**NOTE-21**): confidence{beer} => {nuts} = 1/3 = 0.33
- ¿Por qué no aparece listada la regla `{beer} => {nuts}`? (**NOTE-22**): Por que el algoritmo apriori tiene un paràmetro mìnimo de confidence definido de 0.8, y la regla `{beer} => {nuts}`tiene un valor de la mètrica confidence de 0.33, por lo que no se encuentra dentro del paràmetro definido por el algoritmo apriori.
...
- ¿Cuál es el propósito de `session id`? (**NOTE-23**): Sirve para identificar a la sesiòn a al usuario del cual se tiene data.
- ¿Cuál es el propósito de `track id`? (**NOTE-24**): Sirve para identificar el track que el usuario reprodujo.
- ¿Consideras que las otras columnas del dataset son aplicables para análisis por medio de reglas de asociación, justifica tu respuesta? (**NOTE-25**): Considero que las otras columnas del dataset si nos aplicables para un anàlisis por medio de reglas de asociaciòn, ya que se podrìan encontrar un conjunto de transacciones muy interesantes, las cuales esten relacionadas de una manera muy peculiar, la cual nos permitirà analizar màs a detalle la interacciones de los usuarios con la plataforma de streaming. 
...
- ¿Cuál es el ID y frecuencia absoluta de la sesión más frecuente? (**NOTE-26**): ID: t_bacf06d3-9185-4183-84ea-ff0db51475ce fi = 885 
...
- En el contexto de este dataset con información de sesiones de streaming, ¿cuál es la interpretación de la media aritmética? (**NOTE-28**): Es el promedio de distribución de la longitud del itemset, o más especificamente, el número promedio de track dentro de un itemset.
...
- ¿Qué factores están asociados con que el algoritmo `apriori` con parámetros por omisión no haya producido ninguna regla de asociación, justifica tu respuesta? (**NOTE-29**): Estan asociados los paraametros de support y confidence, ya que apriori tiene un limite infeiror para estos parametros. Asi que si en este caso no se generaron reglas fué por que los valores de support o de confidence no estaban dentro del límite mínimo requerido. 
- ¿Qué riesgos identificas con la asignación de valores muy bajos en los parámetros de  `support` y `confidence` al invocar el algoritmo `apriori`?S Si se asignan valores muy bajos a estos parametros, corremos el riesgo de que las reglas generadas con base en estos no sean lo suficientemente relevantes para nuestro análsis o incluso dichas reglas nocuenten con la frecuencia suficientemente dentro de las transacciones, lo cual significaría que tedriamos más reglas generadas, pero con menos relevancia para su estudio y análisis .  
...
- Provee el enlace al video aquí: **NOTE-30** https://youtu.be/R5x3M9-tL0g 
